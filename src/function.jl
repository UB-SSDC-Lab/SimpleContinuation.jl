...
struct ContinuationFunction{has_full_J,FType,JuType,JλType,JType} <:
    ...
    function ContinuationFunction{has_full_J}(
        ...
        MT = (
            ...
            SubArray{
                ...
                2,
                Matrix{Float64},
                Tuple{Base.Slice{Base.OneTo{Int}},UnitRange{Int}},
                true,
            },
        )
        fargtypes = (
        ...
        Juwrap = FunctionWrappersWrapper(
            Ju, Juargtypes, (Nothing, Nothing, Nothing, Nothing)
        )
        Jλwrap = FunctionWrappersWrapper(
            Jλ, Jλargtypes, (Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
        )

        return new{has_full_J,typeof(fwrap),typeof(Juwrap),typeof(Jλwrap),Nothing}(
    ...
    function ContinuationFunction(
        ...
        return ContinuationFunction{Val{false}}(f, Ju, Jλ)
    end
    ...
    function ContinuationFunction{has_full_J}(
        ...
        Jargtypes = (Tuple{MT,VT[1],VT[1],λT}, Tuple{MT,VT[1],VT[2],λT})

        # Construct function wrappers
        fwrap = FunctionWrappersWrapper(f, fargtypes, (Nothing, Nothing, Nothing, Nothing))
        Jwrap = FunctionWrappersWrapper(J, Jargtypes, (Nothing, Nothing))

        return new{has_full_J,typeof(fwrap),Nothing,Nothing,typeof(Jwrap)}(
    ...
    function ContinuationFunction{has_full_J}(
        ...
        λT = Float64
        VT = (
            Vector{Float64}, SubArray{Float64,1,Vector{Float64},Tuple{UnitRange{Int}},true}
        )
        MT = Matrix{Float64}
        fargtypes = (
            Tuple{VT[1],VT[1],λT},
        ...
        Juargtypes = (Tuple{MT,VT[1],VT[1],λT}, Tuple{MT,VT[1],VT[2],λT})
        Jargtypes = (Tuple{MT,VT[1],VT[1],λT}, Tuple{MT,VT[1],VT[2],λT})
        ...
        return new{has_full_J,typeof(fwrap),typeof(Juwrap),Nothing,typeof(Jwrap)}(
...
struct SparseContinuationFunction{FType,JuType,JType} <: AbstractContinuationFunction
    ...
    function SparseContinuationFunction(
        ...
        return new{typeof(fwrap),typeof(Juwrap),typeof(Jwrap)}(
    ...
    function SparseContinuationFunction(
        ...
        return new{typeof(f),Nothing,Nothing}(
...
function eval_f!(du, u, λ, cf::AbstractContinuationFunction)
    cf.f(du, u, λ)
    return nothing
end
function eval_f!(du, uλ, cf::AbstractContinuationFunction)
    # Get u and λ
    n = length(uλ)
    u = view(uλ, 1:(n - 1))
    λ = uλ[end]
...
function eval_Ju!(J, du, u, λ, cf::ContinuationFunction{hfj,F,Ju}) where {hfj,F,Ju<:Nothing}
    ...
    # Extract Ju
    return J .= view(Jf, :, 1:m)
end
function eval_Ju!(J, du, u, λ, cf::ContinuationFunction)
    cf.Ju(J, du, u, λ)
    return nothing
end
function eval_Ju!(J, du, uλ, cf::ContinuationFunction)
    # Get u and λ
    n = length(uλ)
    u = view(uλ, 1:(n - 1))
    λ = uλ[end]
...
function eval_Ju!(J, du, u, λ, cf::SparseContinuationFunction)
    cf.Ju(J, du, u, λ)
    return nothing
end
function eval_Ju!(J, du, uλ, cf::SparseContinuationFunction)
    # Get u and λ
    n = length(uλ)
    u = view(uλ, 1:(n - 1))
    λ = uλ[end]
...
function eval_J!(J, du, u, λ, cf::ContinuationFunction{hfj}) where {hfj<:Val{false}}
...
function eval_J!(J, du, u, λ, cf::ContinuationFunction)
    cf.J(J, du, u, λ)
    return nothing
end
function eval_J!(J, du, uλ, cf::ContinuationFunction)
    # Get u and λ
    n = length(uλ)
    u = view(uλ, 1:(n - 1))
    λ = uλ[end]
...
function eval_J!(J, du, u, λ, cf::SparseContinuationFunction)
    cf.J(J, du, u, λ)
    return nothing
end
function eval_J!(J, du, uλ, cf::SparseContinuationFunction)
    # Get u and λ
    n = length(uλ)
    u = view(uλ, 1:(n - 1))
    λ = uλ[end]
