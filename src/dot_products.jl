...
function (d::ScaledDotProduct)(u1::AbstractArray, u2::AbstractArray, λ1, λ2)
    θ = d.θ
    return θ * dot(u1, u2) + (1.0 - θ) * λ1 * λ2
end
function (d::DoubleScaledDotProduct)(u1::AbstractArray, u2::AbstractArray, λ1, λ2)
    tθ = 2.0 * d.θ
    return tθ * dot(u1, u2) + (2.0 - tθ) * λ1 * λ2
end
function (d::BifurcationKitDotProduct)(u1::AbstractArray, u2::AbstractArray, λ1, λ2)
    n = length(u1)
    θ = d.θ
...
function palc_norm(δu, δu0, δλ, δλ0, ds, d::AbstractDotProduct)
    return d(δu, δu0, δλ, δλ0) - ds
end

# The norm partial wrt δu
function palc_norm_dδu!(dnorm_du, δu0, d::AbstractDotProduct)
    return ddotdu1!(dnorm_du, δu0, d)
end

# The norm partial wrt λ
function palc_norm_dδλ(δλ0, d::AbstractDotProduct)
