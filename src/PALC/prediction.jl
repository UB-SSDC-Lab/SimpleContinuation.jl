...
# ===== Secant Predictor
function palc_prediction!(
    cache::PALCCache, alg::PALC{Secant}, p::ContinuationProblem, solvers, trace
)
    ...
    if length(cache.br) < 2
        ...
        sdot = sign(
            alg.inner_prod(cache.δu0, view(cache.δuλ0, 1:n), cache.δλ0, cache.δuλ0[n + 1])
        )
