...
struct PALCSolverCache{LP,NNLP,PNLP}
    ...
    function PALCSolverCache(
        ...
        n_nlp, palc_nlp = construct_nlp_caches(p, alg, cache, newton_iter, newton_tol)

        return new{typeof(lp),typeof(n_nlp),typeof(palc_nlp)}(
            lp, n_nlp, palc_nlp, newton_iter, newton_tol, newton_max_resid
        )
    end
end
...
function construct_nlp_caches(
    p::ContinuationProblem, alg::PALC, cache::PALCCache, newton_iter, newton_tol
)
...
function construct_nlp_caches(
    p::ContinuationProblem{<:SparseContinuationFunction{F,Ju,J}},
    alg::PALC,
    cache::PALCCache,
    newton_iter,
    newton_tol,
...
function construct_nlp_caches(
    ...
    n_nlp = init(
        NonlinearProblem{true}(
            NonlinearFunction{true,SciMLBase.FullSpecialize}(
                (du, u, p) -> eval_f!(du, u, p[2].λn, p[1]); jac_prototype=Ju_prototype
            ),
            cache.u0,
            nlp_params,
        ),
    ...
    palc_nlp = init(
        NonlinearProblem{true}(
            NonlinearFunction{true,SciMLBase.FullSpecialize}(
                palc_correction_function!; jac_prototype=Jp
            ),
            cache.uλ0,
            nlp_params,
        ),
...
function solve_natural_nlp!(solvers::PALCSolverCache, u0, trace)
    ...
    reinit!(solvers.n_nlp, u0; maxiters=maxiters, abstol=abstol, reltol=reltol)
...
function solve_palc_nlp!(solvers::PALCSolverCache, uλ0, trace)
    ...
    # Reinitialize the nonlinear problem
    reinit!(solvers.palc_nlp, uλ0; maxiters=maxiters, abstol=abstol, reltol=reltol)
...
function print_natural_solve_trace(
    solvers::PALCSolverCache, trace::ContinuationAndNewtonSteps
)
